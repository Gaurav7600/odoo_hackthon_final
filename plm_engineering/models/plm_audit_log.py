...
class PlmEcoApproval(models.Model):
    ...
    eco_id = fields.Many2one(
        'plm.eco', string='ECO', required=True, ondelete='cascade', index=True,
    )
    eco_reference = fields.Char(
        related='eco_id.reference', string='ECO Reference', store=True, readonly=True,
    )
...
class PlmAuditLog(models.Model):
    ...
    eco_id = fields.Many2one(
        'plm.eco', string='ECO', ondelete='set null', index=True,
    )
    eco_reference = fields.Char(
        related='eco_id.reference', string='ECO Ref', store=True, readonly=True,
    )
    ...
    change_summary = fields.Char(
        compute='_compute_summary', string='Change Summary',
    )
